...
class SwarmAgentSDK extends EventEmitter {
  ...
  handleMessage(message: BaseMessage): void {
    ...
    if (message.type === 'task.execute') {
      this.handleTask(message as TaskExecuteMessage);
      return;
    }
   ...
   */
  async executeService(serviceName: string, params: Record<string, any> = {}, timeout = 30000): Promise<any> {
   ...
   * @param timeout Request timeout
   */
  async executeMCPTool(serverId: string, toolName: string, parameters: Record<string, any> = {}, timeout = 60000): Promise<any> {
    const message: BaseMessage = {
      id: uuidv4(),
   ...
   * @param timeout Request timeout
   */
  async executeTool(toolName: string, parameters: Record<string, any> = {}, serverId: string | null = null, timeout = 60000): Promise<any> {
   ...
   * @param timeout Request timeout
   */
  async executeAgentTask(targetAgentName: string, taskType: string, taskData: Record<string, any> = {}, timeout = 30000): Promise<any> {
  ...
  async sendTaskNotification(notification: any): Promise<void> {
    ...
    try {
      await this.send(notificationMessage as BaseMessage);
    } catch (error) {
  ...
  async executeServiceTask(
    ...
    if (options.onNotification) {
      ...
      const resultHandler = (notificationMessage: any) => {
