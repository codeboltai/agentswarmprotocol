...
const agent = new SwarmAgentSDK({
  agentId: 'child-agent-002', // Consistent agent ID for reconnections
  name: 'GrandChildAgent',
  description: 'A grandchild agent that executes tasks delegated from parent agents',
...
agent.onTask(async (taskData: any, message: TaskExecuteMessage) => {
  ...
  try {
    ...
    agent.sendTaskMessage(taskId, {
      type: 'progress',
      message: `Grand Child agent started processing ${taskType} task`,
    ...
    agent.sendTaskMessage(taskId, {
      type: 'progress',
      message: `Grand Child agent completed processing ${taskType} task`,
